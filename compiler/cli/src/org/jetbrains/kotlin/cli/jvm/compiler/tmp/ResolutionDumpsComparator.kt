...
class ResolutionDumpsComparator {
    fun compareDumpsForFile(before: RDumpForFile, after: RDumpForFile) {
        check(before == after) { "Trying to compare dumps for different files" }

        val offsetToElementMapBefore = before.results.map { it.offset to it }.toMap()
        val offsetToElementMapAfter = after.results.map { it.offset to it }.toMap()
        ...
        for (offset in allOffsets) {
            ...
            val difference = when {
                ...
                else -> throw IllegalStateException("Can't compare resolution elements: both are null")
            }
    ...
    private fun compareResolutionElements(before: RDumpElement, after: RDumpElement): ResolutionDifference? {
        ...
        if (before::class != after::class) {
            return ResolutionDifference(before.presentableText, ResolutionKindMismatch(before, after))
        }
...
// E.g. Resolved -> Error
class ResolutionKindMismatch(before: RDumpElement, after: RDumpElement) :
    MismatchReason(
