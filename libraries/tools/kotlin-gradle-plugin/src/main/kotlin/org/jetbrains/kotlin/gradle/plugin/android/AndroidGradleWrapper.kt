...
object AndroidGradleWrapper {
    ...
    fun getAnnotationProcessorOptionsFromAndroidVariant(variantData: Any?): Map<String, String>? {
        ...
        return compileOptions("getAnnotationProcessorOptions")("getAnnotationProcessorOptions")("getArguments") as? Map<String, String>
    }
...
private operator fun Any?.invoke(methodName: String, isStatic: Boolean = false): Any? {
    if (this == null) {
        return null
    }

    fun Array<Method>.findMethod() =
        singleOrNull { it.name == methodName && it.parameterCount == 0 && Modifier.isStatic(it.modifiers) == isStatic }
    ...
    try {
        ...
        return methodToInvoke.invoke(if (isStatic) null else this, methodName)
...
private operator fun Any?.get(name: String, isStatic: Boolean = false): Any? {
    if (this == null) {
        return null
    }

    fun Array<Field>.findField() = singleOrNull { it.name == name && Modifier.isStatic(it.modifiers) == isStatic }
    ...
    try {
        ...
        return fieldToInvoke.get(if (isStatic) null else this)
    } catch (e: InvocationTargetException) {
        throw e.targetException
    } finally {
