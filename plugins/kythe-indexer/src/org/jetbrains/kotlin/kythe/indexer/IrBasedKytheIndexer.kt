...
class IrBasedKytheIndexer(
    ...
    private val visitedTypes: HashMap<KotlinType, Storage.VName> = hashMapOf()
    ...
    override fun visitClass(declaration: IrClass, data: Nothing?): Storage.VName {
        return visited.getOrPut(declaration) {
            ...
            val classVName = processDeclaration(declaration) {
                for (member in declaration.declarations) {
                    member.accept(this, data)?.emitChildOf(it)
                }

                for (superClassSymbol in declaration.superClasses) {
                    val superVName = visitClass(superClassSymbol.owner, data)
                    kotlinEntrySets.emitEdge(it, EdgeKind.EXTENDS, superVName)
                }
            }
    ...
    override fun visitFunction(declaration: IrFunction, data: Nothing?): Storage.VName? {
        ...
        return visited.getOrPut(declaration) {
            ...
            val functionVName = processDeclaration(declaration) {
    ...
    override fun visitProperty(declaration: IrProperty, data: Nothing?): Storage.VName =
        visited.getOrPut(declaration) {
            ...
            val propertyVName = processSimpleTypedDeclaration(declaration, declaration.type) {
    ...
    override fun visitTypeParameter(declaration: IrTypeParameter, data: Nothing?): Storage.VName =
        visited.getOrPut(declaration) {
            ...
            val upperBoundVNames = declaration.upperBounds
    ...
    // NB. We're mimicking visitor-API for KotlinType because we're expecting to migrate to IR-types soon
    fun visitType(kotlinType: KotlinType, context: IrTypeParametersContainer?): Storage.VName {
        visitedTypes[kotlinType]?.let { return it }
        return kotlinEntrySets.getVNameForTypeReference(kotlinType, context).vNameForWholeType
    }
    ...
    override fun visitFunctionAccess(expression: IrFunctionAccessExpression, data: Nothing?): Storage.VName {
        val owner: IrDeclaration? = expression.symbol.owner as? IrDeclaration
