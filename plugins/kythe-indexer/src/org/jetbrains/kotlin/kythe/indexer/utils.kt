...
fun IrDeclaration.toNodeKind(): NodeKind {
    return when (this) {
        is IrModuleFragment -> throw IllegalStateException("Modules doesn't have corresponding Kythe node (yet?)")
        is IrFile -> NodeKind.FILE
        is IrClass -> this.irClassKind.toNodeKind()
...
fun IrClassKind.toNodeKind(): NodeKind = when (this) {
    IrClassKind.CLASS -> NodeKind.RECORD_CLASS
    IrClassKind.INTERFACE -> NodeKind.INTERFACE
    IrClassKind.ENUM_CLASS -> NodeKind.SUM_ENUM_CLASS
    IrClassKind.ENUM_ENTRY -> NodeKind.CONSTANT
    IrClassKind.ANNOTATION_CLASS -> throw UnsupportedOperationException("Annotations are not supported yet")
    IrClassKind.OBJECT -> NodeKind.OBJECT
    IrClassKind.COMPANION -> NodeKind.COMPANION
}
